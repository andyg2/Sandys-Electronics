...
Ships with **197 inventory items** across 12 categories and **39 example projects** spanning absolute-beginner LED games through intermediate ESP32 / MQTT builds.
...
The first start runs `seed.php` and loads `inventory/examples/projects.json` automatically. Subsequent starts are no-ops. To wipe and re-seed: `docker compose down -v && docker compose up -d`.
...
`setup.sh` checks for PHP 8.2+ and the `pdo_mysql` extension, creates `inventory/src/config.php` from the example, creates the database if it doesn't exist, runs the seed, and optionally loads the 39 example projects.
...
make examples      # load 39 example projects into existing schema
...
.
  ...
  inventory/
    ...
    examples/
      ...
      projects.json      39 example projects with allocations + tags
